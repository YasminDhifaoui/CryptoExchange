...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($errors)) {
        try {
            $insertQuery = "INSERT INTO pair_coin (name, fullname, market_name, cryptocoin_name, initial_price, status) 
                            VALUES (?, ?, ?, ?, ?, ?)";
            $insertStmt = $pdo->prepare($insertQuery);
            $insertStmt->execute([$name, $fullname, $market_name, $cryptocoin_name, $initial_price, $status]);
            ...
            <form action="" method="POST">
                ...
                <select name="market_name" id="market_name" required>
                    ...
                    <?php foreach ($markets as $market): ?>
                        <option value="<?php echo htmlspecialchars($market['name']); ?>" <?php echo (isset($market_name) && $market_name == $market['name']) ? 'selected' : ''; ?>>
                ...
                <select name="cryptocoin_name" id="cryptocoin_name" required>
                    ...
                    <?php foreach ($cryptocoins as $coin): ?>
                        <option value="<?php echo htmlspecialchars($coin['name']); ?>" <?php echo (isset($cryptocoin_name) && $cryptocoin_name == $coin['name']) ? 'selected' : ''; ?>>
                ...
                <input type="number" id="initial_price" name="initial_price" step="0.001" min="0" value="<?php echo isset($initial_price) ? htmlspecialchars($initial_price) : ''; ?>" required>
                ...
                <input type="submit" value="Create Pair Coin">
            </form>

            <a href="pairCoin_list.php" class="back-link">Back to Pair Coin List</a>
        </div>
    </main>
